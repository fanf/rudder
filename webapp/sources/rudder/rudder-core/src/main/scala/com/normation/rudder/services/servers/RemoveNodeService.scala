...
class RemoveNodeServiceImpl(
  ...
  def findNodeStatus(nodeId: NodeId): IOResult[Set[InventoryStatus]] = {
    for {
      con <- ldap
      ...
      deleted      <- ZIO.foreach(nodeGroupIds) { nodeGroupId =>
                        for {
                          ...
                          diff         <- (if(nodeGroup.isSystem) {
                                          ...
                                          }).chainError(s"Could not update group '${nodeGroup.id.value}' to remove node '${nodeId.value}'")
                        } yield {
                          diff
                        }
                      }
...
class KeyStatus(ldap: LDAPConnectionProvider[RwLDAPConnection], deletedDit: InventoryDit) extends PostNodeDeleteAction {
  override def run(nodeId: NodeId, mode: DeleteMode, info: Option[NodeInfo], status: Set[InventoryStatus]): UIO[Unit] = {
    if(mode == DeleteMode.MoveToRemoved) {
