...
object UserRepository {
  ...
  def computeUpdatedUserList(
    ...
    // resurrected are zombie that get back to live with the new origin, but disabled
...
class JdbcUserRepository(doobie: Doobie) extends UserRepository {
  ...
  private[users] def predicateLogDate(lastLogin: Option[DateTime]) = lastLogin match {
    case None        => None
  ...
  private[users] def predicateNotOrigin(origins: List[String]) = origins match {
    case Nil    => None
  ...
  private[users] def select(
    ...
    val selectUsers = List(predicateUser(userIds), predicateLogDate(notLoggedSince)).flatten

    if (selectUsers.isEmpty && defaultToNone) {
      connectionUnit.map(_ => Nil)
    } else {
      (
        sql"select id, statushistory from users " ++ Fragments.whereAndOpt(
          if (selectUsers.isEmpty) None else Some(Fragments.or(selectUsers: _*)),
          predicateNotOrigin(excludeFromOrigin),
          andSelectFragment
        )
      ).query[(String, List[StatusHistory])].to[List]
    }
  }
