...
final object PolicyTechnique {
  def forAgent(technique: Technique, agentType: AgentType): Either[String, PolicyTechnique] = {
    technique.agentConfigs.find( _.agentType == agentType) match {
      case None    => Left(s"Error: Technique '${technique.name}' (${technique.id.show}) does not support agent type '${agentType.displayName}'")
      ...
      trackerVariable.spec.boundingVariable match {
        ...
        case Some(value) =>
          originalVars.get(value) match {
            ...
            case None => throw new NotFoundException("No valid bounding found for trackerVariable " + trackerVariable.spec.name + " found in directive " + id.directiveRId.show)
