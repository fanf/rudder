...
private object InternalChangeEvent {
  ...
  final case class Update[A](oldNode: A, newNode: A) extends InternalChangeEvent[A]
...
//  } yield ()).runNow
  ...
  private def checkRootProperties(node: NodeFact): IOResult[Unit] = {
    ...
    def rootIsEnabled(node: NodeFact): ValidationResult = {
      ...
      for {
        ...
        e <- nodeFact.rudderSettings.status match {
               ...
               case PendingInventory  =>
                 saveOn(pendingNodes, nodeFact.toCore).map { e =>
                   e match {
                     case InternalChangeEvent.Create(node)      => NodeFactChangeEventCC(NewPending(node), cc)
                     case InternalChangeEvent.Update(old, next) => NodeFactChangeEventCC(UpdatedPending(old, next), cc)
               ...
               case AcceptedInventory =>
                 saveOn(acceptedNodes, nodeFact.toCore).map { e =>
                   e match {
                     ...
                     case InternalChangeEvent.Update(old, next) => NodeFactChangeEventCC(Updated(old, next), cc)
                     case InternalChangeEvent.Delete(node)      => NodeFactChangeEventCC(Deleted(node), cc)
                     case InternalChangeEvent.Noop(nodeId)      => NodeFactChangeEventCC(Noop(nodeId), cc)
                   }
                 }
