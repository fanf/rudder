  ...
  def buildPolicy(nodeInfo: NodeInfo, mode: GlobalPolicyMode, boundedPolicyDrafts: Seq[BoundPolicyDraft]): Box[List[Policy]] = {
    ...
    val deduplicateDrafts = boundedPolicyDrafts.groupBy(x => (x.id.ruleId, x.id.directiveRId)).map { case (draftId, seq) =>
      ...
      if(seq.lengthCompare(1) > 0) {
        PolicyGenerationLogger.error(s"The directive '${seq.head.id.directiveRId.show}' on rule '${seq.head.id.ruleId.value}' was added several times on node " +
    ...
    val keptUniqueDraft = groupedDrafts.uniqueInstance.map { case (techniqueName, drafts) =>
      ...
      if(differentDirectives.size > 1) {
        PolicyGenerationLogger.warn(s"Unicity check: NON STABLE POLICY ON NODE '${nodeInfo.hostname}' for mono-instance (unique) technique " +
                                    s"'${keep.technique.id.show}'. Several directives with same priority '${keep.priority}' are applied. " +
                                    s"Keeping (ruleId@@directiveId) '${keep.id.ruleId.value}@@${keep.id.directiveRId.show}' (order: ${keep.ruleOrder.value}/" +
                                    s"${keep.directiveName}, discarding: ${samePriority.tail.map(x => s"${x.id.ruleId.value}@@${x.id.directiveRId.show}:"+
            s"${x.ruleName}/${x.directiveName}").mkString("'", "', ", "'")}")
      }
      PolicyGenerationLogger.trace(s"Unicity check: on node '${nodeInfo.id.value}' for mono-instance (unique) technique '${keep.technique.id.show}': " +
                                   s"keeping (ruleId@@directiveId) '${keep.id.ruleId.value}@@${keep.id.directiveRId.show}', discarding less priorize: " +
                                   s"${lesserPriority.map(x => x.id.ruleId.value+"@@"+x.id.directiveRId.show).mkString("'", "', ", "'")}")
    ...
    PolicyGenerationLogger.trace(s"'${nodeInfo.id.value}': to merge multi-directives: [${groupedDrafts.toMerge.map{ case (k, v) => s"${k.value}: ${v.map(displayDraft).mkString(" | ")}"}.mkString("] [")}]")
