...
final object QSMapping {
  ...
  val attributeNames = {
    ...
    QSAttribute.all.map { a =>
      a match {
        ...
        case OsName            =>
          (a, Set(OsName.name, "osname", "os")) // not also full name because osFullname contains osName, not the reverse
        ...
        case DirectiveVarName  =>
          (a, Set(DirectiveVarName.name, "dir_param", "dir_parameter", "dir_parameters", "param", "parameter", "parameters"))
        case DirectiveVarValue =>
          (
            a,
            Set(
              DirectiveVarValue.name,
              "dir_param",
              "dir_parameter",
              "dir_parameters",
              "param",
        ...
        case ParameterName     =>
          (a, Set(ParameterName.name, "parameter_name", "parametername", "paramname", "name", "parameter", "param", "id"))
        case ParameterValue    =>
          (a, Set(ParameterValue.name, "parameter_value", "parametervalue", "paramvalue", "value", "parameter", "param"))
  ...
  val attributeNameMapping: Map[String, Set[QSAttribute]] = {
    ...
    val byNames: Map[String, Seq[(String, QSAttribute)]] = attributeNames.toSeq.flatMap {
      case (a, names) => names.map(n => (n.toLowerCase, a))
    }.groupBy(_._1)
