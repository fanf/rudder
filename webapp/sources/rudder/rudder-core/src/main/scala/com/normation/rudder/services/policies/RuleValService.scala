...
class RuleValServiceImpl(
  ...
  def getParsedPolicyDraft(id: DirectiveRId, ruleId: RuleId, ruleOrder: BundleOrder, ruleName: String, directiveLib: FullActiveTechniqueCategory) : Box[Option[ParsedPolicyDraft]]= {
    directiveLib.allDirectives.get(id) match {
      case None => Failure(s"Cannot find directive with id '${id.show}' when building rule '${ruleOrder.value}' (${ruleId.value})")
      case Some((_, directive) ) if !(directive.isEnabled) =>
        logger.debug("The Directive with id %s is disabled and we don't generate a ParsedPolicyDraft for Rule %s".format(id.show, ruleId.value))
        Full(None)
        ...
        for {
          ...
          exists <- {
            if (vared.isDefinedAt(technique.trackerVariableSpec.name)) {
              ...
              logger.error("Cannot find key %s in Directive %s when building Rule %s".format(technique.trackerVariableSpec.name, id.show, ruleId.value))
              Failure("Cannot find key %s in Directibe %s when building Rule %s".format(technique.trackerVariableSpec.name, id.show, ruleId.value))
            }
          }
