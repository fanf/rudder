  ...
  override def targetDependencies(target:RuleTarget, onlyEnableable:Boolean = false) : Box[TargetDependencies] = {
    ...
    def filterRules(rules:Seq[Rule]) : IOResult[Seq[Rule]] = {
        ...
        ZIO.foreach(enabledCr.groupBy { case (rule,idAndRev) => idAndRev }.toSeq) { case (id, seq) =>
          for {
            optPair <- roDirectiveRepository.getActiveTechniqueAndDirective(id).chainError(s"Error when retrieving directive with ID '${id.show}'")
            ...
            res     <- optPair match {
                         case None =>
                           logPure.warn(s"Directive with id '${id.show}' is referenced in rules with names: '${seq.map(r =>r._1.name).mkString("','")}' but it was not found. Perhaps these rules should be updated (saved again).") *>
                           Seq().succeed
