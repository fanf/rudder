...
class TestBuildNodeConfiguration extends Specification {
  ...
  "build node configuration" in {
    ...
    for(i <- 0 until 10) {
      ...
      BuildNodeConfiguration.buildNodeConfigurations(activeNodeIds, ruleVals, nodeContexts.ok, allNodeModes, Map(), scriptEngineEnabled, globalPolicyMode, maxParallelism, jsTimeout, generationContinueOnError)
...
//
//    def nano = UIO.succeed(System.nanoTime)
//    def log(s: String, t1: Long, t2: Long) = UIO.succeed(logger.trace(s + s"${(t2-t1)/1000} µs"))
//
//    val count = 0 until 1
//    val prog =
...
//        _    <- ref.get.flatMap(t => UIO.succeed(logger.trace(s"inner sum $j: ${t/1000} µs")))
//      } yield ()
//    }
