...
trait UserRepositoryTest extends Specification with Loggable {
  ...
  "basic sequential operations with users" >> {
    ...
    val userInfosBobOidc = {
       ...
       * - Bob is in the list, but with a status "disabled" - we don't allow user to resurrect active
  ...
  "testing purge" >> {
    ...
    "deleting+purging all users not logged in since a date in the future should remove all users" >> {
      ...
      (repo.delete(Nil, Some(dateInit.plusYears(1)), Nil, EventTrace(actor, dateInit)) *>
      repo.purge(Nil, Some(dateInit.plusSeconds(1)), Nil, EventTrace(actor, DateTime.now()))).runNow
