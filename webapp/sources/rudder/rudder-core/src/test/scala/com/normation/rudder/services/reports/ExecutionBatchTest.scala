...
class ExecutionBatchTest extends Specification {
  private implicit def str2directiveId(s:String) = DirectiveRId(DirectiveId(s))
  ...
  def buildDataForMergeCompareByRule(
    ...
    val dirPerRule = ruleIds.map(rule => (RuleId(rule), directiveIds.map(dir => DirectiveRId(DirectiveId(dir + "@@"+rule)))))
