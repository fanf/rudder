...
object TestCompliance {

  val cs       = SetUpCompliance.complianceAPIService
  ...
  def main(args: Array[String]): Unit = {
    ...
    val file = "/home/fanf/NORMATION/Support/BMW/2025-03-performance-queue-compliance/compliance_computation/%p.html"
    profiler.execute(s"start,event=cpu,tree,file=${file}")
    println(s"computing compliance...")

    // profiler.execute(s"start,event=cpu,alloc=128,tree,file=${file}")
