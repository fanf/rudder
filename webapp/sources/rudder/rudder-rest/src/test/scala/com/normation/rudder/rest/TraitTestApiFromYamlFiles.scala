...
object TraitTestApiFromYamlFiles {
  ...
  def buildLiftRules[A <: LiftApiModuleProvider[_ <: EndpointSchema]](modules: List[A], versions: List[ApiVersion], userService: Option[UserService]) = {
    implicit val userServiceImp = userService match {
      case None => new UserService {
                      val user = new User {
    ...
    val rudderApi = new LiftHandler(apiDispatcher, versions, new AclApiAuthorization(LiftApiProcessingLogger, userServiceImp, () => apiAuthorizationLevelService.aclEnabled), None)
    modules.foreach { module =>
      rudderApi.addModules(module.getLiftEndpoints())
    }
