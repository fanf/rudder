...
class NodeConfigurationServiceImpl(
   ...
   *
   */
  def writeTemplate(rootNodeId: NodeId, nodesToWrite: Set[NodeId], allNodeConfigs: Map[NodeId, NodeConfiguration], versions: Map[NodeId, NodeConfigVersion]) : Box[Seq[NodeConfiguration]] = {
