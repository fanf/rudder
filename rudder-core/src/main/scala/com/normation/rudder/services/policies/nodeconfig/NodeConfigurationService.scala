...
trait NodeConfigurationService {
   ...
   */
  def writeTemplate(rootNodeId: NodeId, configToWrite: Set[NodeId], allNodeConfigs: Map[NodeId, NodeConfiguration], versions: Map[NodeId, NodeConfigVersion]): Box[Seq[NodeConfiguration]]
