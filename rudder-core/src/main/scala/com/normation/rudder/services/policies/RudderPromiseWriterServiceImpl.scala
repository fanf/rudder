...
class RudderCf3PromisesFileWriterServiceImpl(
   ...
   */
  override def writePromisesForMachines(configToWrite: Set[NodeId], rootNodeId: NodeId, allNodeConfigs:Map[NodeId, NodeConfiguration], versions: Map[NodeId, NodeConfigVersion]): Box[Seq[PromisesFinalMoveInfo]] = {
  ...
  private[this] def prepareRulesForAgents(
    ...
    , nodeConfigVersion      : NodeConfigVersion
