...
trait DeploymentService extends Loggable {
  ...
  def deploy() : Box[Seq[NodeConfiguration]] = {
    ...
    val result = for {
      ...
      allParameters   <- getAllGlobalParamters ?~! "Could not get global parameters"
  ...
  def getAllGlobalParamters: Box[Seq[GlobalParameter]]
...
trait DeploymentService_findDependantRules_bruteForce extends DeploymentService {
  ...
  override def getAllGlobalParamters: Box[Seq[GlobalParameter]] = parameterService.getAllGlobalParameters()
}
