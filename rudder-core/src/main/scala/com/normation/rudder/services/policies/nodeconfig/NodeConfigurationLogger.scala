...
class NodeConfigurationLoggerImpl(
...

  {
    ...
    if(p.exists() && p.canWrite()) {
      ...
      logger.error(s"Error when trying to create the directory where node configuration are saved: '${path}'. Please check that that file is a directory with write permission for Rudder.")
    }
  }
  ...
  def log(nodeConfiguration: Seq[NodeConfiguration]): Box[Set[NodeId]] = {
    ...
    if(doLogFile.exists) {
      ...
      logger.debug(s"File '${doLogFile.getAbsolutePath}' does not exists: don't log node configuration")
      Full(Set())
    }
  }
