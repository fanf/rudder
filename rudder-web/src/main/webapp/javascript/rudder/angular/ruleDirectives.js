...
ruleDirectives.controller('DirectiveCtrl', ['$scope', function($scope) {
    ...
    $scope.init = function ( directives ) {
    ...
    $scope.directivesIsEmpty = function() {
      Object.getOwnPropertyNames($scope.directives).length === 0;
    }  
    ...
    $scope.removeInclude = function ( directiveId ) {
      ...
      var jsId = directiveId.replace(':','\\:');
      $("#jstree-"+jsId).removeClass("directiveIncluded");
    };
    ...
    $scope.addInclude = function ( directiveId, directiveName ) {
      ...
      var jsId = directiveId.replace(':','\\:');
      $("#jstree-"+jsId).addClass("directiveIncluded");
      $scope.updateDirective();
    };
    ...
    $scope.toggleDirective = function ( directiveId, directiveName ) {
     ...
     $scope.directiveAddExplanation = [ "<h3>Add Directives here to so that they will be applied to the Rule targets.</h3>"
                                  ].join(" ");
  } ] ) ;
