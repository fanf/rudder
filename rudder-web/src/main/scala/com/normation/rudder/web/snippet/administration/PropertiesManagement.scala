...
class PropertiesManagement extends DispatchSnippet with Loggable {
  ...
  def changeMessageConfiguration = { xml : NodeSeq =>
    ...
    // initial values
    val initEnabled = configService.rudder_ui_changeMessage_enabled
    val initMandatory = configService.rudder_ui_changeMessage_mandatory
    val initExplanation = configService.rudder_ui_changeMessage_explanation
    ...
    def submit = {
      ...
      configService.set_rudder_ui_changeMessage_mandatory(mandatory)
      configService.set_rudder_ui_changeMessage_explanation(explanation)
    ...
    val initEnabled = configService.rudder_workflow_enabled
    var enabled = initEnabled.getOrElse(false)
    val initSelfVal = configService.rudder_workflow_self_validation
    var selfVal = initSelfVal.getOrElse(false)
    val initSelfDep = configService.rudder_workflow_self_deployment
    var selfDep = initSelfDep.getOrElse(false)


    def submit = {
      configService.set_rudder_workflow_enabled(enabled)
      configService.set_rudder_workflow_self_validation(selfVal)
      configService.set_rudder_workflow_self_deployment(selfDep)
