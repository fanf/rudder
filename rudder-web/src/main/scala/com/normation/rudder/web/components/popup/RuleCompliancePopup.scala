...
class RuleCompliancePopup(
  ...
  def showReportsByType(
    ...
    val unknowns = reports.map( r => (r.nodeId, r.getValues( v => v.status == UnknownReportType)))

    val missing = unknowns.flatMap( _._2).filter( _._3.messages.forall(_.message.isEmpty))
    val unexpected = unknowns.flatMap { case (nodeId, seq) =>
      seq.filter(_._3.messages.exists(_.message.nonEmpty)).map { case(a,b,c) => (nodeId, a, b, c) }
    }
