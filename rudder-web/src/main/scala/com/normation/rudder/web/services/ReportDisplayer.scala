...
class ReportDisplayer(
  ...
  private[this] def displayReports(node : NodeInfo) : NodeSeq = {
    val boxXml = (
      for {
        ...
        val unexpected = getUnexpectedComponents(filteredReports, directiveLib)
        bind("lastReportGrid",reportByNodeTemplate
          , "intro"      ->  intro
          , "grid"       -> showReportDetail(filteredReports, node)
          , "missing"    -> showMissingReports(unexpected)
  ...
  private[this] def getUnexpectedComponents(nodeStatusReports: Seq[RuleNodeStatusReport], directiveLib: FullActiveTechniqueCategory) = {
   /*
    * Note:
    * - missing reports ar unexpected without error message
    ...
    */
    for {
      ...
      directiveReport <- ruleNodeReport.directives.values
      cptReport       <- directiveReport.components.values
      valueReport     <- cptReport.componentValues.values
      if(valueReport.status == UnknownReportType && valueReport.messages.exists(m => m.message.nonEmpty))
    } yield {
      val (techName, techVersion) = directiveLib.allDirectives.get(directiveReport.directiveId).map { case(tech,dir) =>
        (tech.techniqueName.value, dir.techniqueVersion.toString)
      }.getOrElse(("Unknown technique", "N/A"))

      ((cptReport.componentName, valueReport.componentValue, valueReport.messages.flatMap(_.message)), techName, techVersion)
    }
  }
