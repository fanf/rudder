...
class SettingsAPI8(
  ...
  def response ( function : Box[JValue], req : Req, errorMessage : String)(implicit action : String) : LiftResponse = {
    RestUtils.response(restExtractor, kind)(function, req, errorMessage)
  }
  override def requestDispatch(apiVersion : ApiVersion): PartialFunction[Req, () => Box[LiftResponse]] = {
    case Get("global_policy_mode" :: Nil, req) => {
      ...
      RestUtils.response(restExtractor, "settings")(mode, req, "Could not get global policy mode")("getSetting")
    }
    case Post("global_policy_mode" :: Nil, req) => {
      ...
      RestUtils.response(restExtractor, "settings")(newValue, req, "Could not get global policy mode")("setSetting")
    }
